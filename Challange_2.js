...
let calcAverageHumanAge=function(ages){
    ...
    let avg=ages.map((val,idx)=>{
        if(val<=2){
            return 2*val;
        }else return (val*4)+16;
    }).filter((val,idx)=>{
        if(val>18) return val;
    })
    return avg.reduce((acc,curr,idx)=>{
        // console.log( `Current Acc=${acc} at an index of ${idx}` )
        return acc+curr;
    },0)/avg.length;
    

}
const ans=calcAverageHumanAge(ages)
console.log( ans )
